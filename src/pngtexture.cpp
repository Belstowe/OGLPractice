...
PNGTexture::PNGTexture(const GLchar *name) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, hasAlpha ? GL_RGBA : GL_RGB, width, height, 0,
               hasAlpha ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, bufImage);
  glGenerateMipmap(GL_TEXTURE_2D);
  free(bufImage);
  glBindTexture(GL_TEXTURE_2D, 0);
}

bool PNGTexture::load(const GLchar *name, int &outWidth, int &outHeight,
  ...
  // Непосредственно запись изображения по указателям; STRIP_16 - стандартный,
  // PACKING - использование 8-битной глубины цвета, EXPAND - расширяет цветовую
  // палитру RGB
  ...
  // Цветовой тип позволяет определить, используется ли RGB или же RGBA (с доп.
  // атрибутом прозрачности)
  ...
  *outData = (unsigned char *)malloc(row_bytes * outHeight);
  ...
  for (int i = 0; i < outHeight; i++) {
    memcpy(*outData + (row_bytes * (height - 1 - i)), row_pointers[i],
           row_bytes);
  }
...
void PNGTexture::bind() { glBindTexture(GL_TEXTURE_2D, this->texture); }
