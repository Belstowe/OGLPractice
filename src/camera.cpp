...
Camera::Camera()
    : pos(glm::vec3(-2.0f, 3.5f, 4.3f)), front(glm::vec3(0.4f, -0.5f, -0.75f)),
      up(glm::vec3(0.25f, 0.85f, -0.4f)), worldUp(glm::vec3(0.0f, 1.0f, 0.0f)),
      yaw(-60.0f), pitch(-30.0f), speed(3.0f), mouseSensitivity(0.25f),
      zoom(45.0f) {
  this->updateVectors();
}
...
void Camera::move(__camera_movement direction, GLfloat deltatime) {
  ...
  case DOWNTURN:
    ...
    break;
  case LEFTTURN:
    this->yaw -= 30.0f * this->speed * deltatime;
    this->updateVectors();
    break;
  case RIGHTTURN:
    this->yaw += 30.0f * this->speed * deltatime;
...
void Camera::controlms(GLfloat xoffset, GLfloat yoffset,
